...
class SUPAModel:
    ...
    def __init__(self, csv_param):##从 CSV 文件中读取参数（csv_param），使用 pandas 进行数据处理。
...
#        #self.BacktestInit = Dictionary_BacktestInit# parameters
    def ForwardSimulation(self, NumOfSim, NumOfTime, freq, back_test):
       # 实现核心模拟功能，使用随机微分方程对经济和金融变量进行前向模拟。
        # back_test == 0 forward simulation, back_test == 1, true. It controls differenti initial values 根据 back_test 模式设置变量的初始值。
        # freq : number of time steps per year
        # dt: time step
        NumOfTime = NumOfTime+1 #NumOfTime：时间步数。
        self.dt=1.0/freq
        dt=self.dt
        sqrt_dt=np.sqrt(dt)

        # add extra sim for median
        if NumOfSim > 1: #NumOfSim：模拟路径（情景）数量。
            NumOfSim = NumOfSim+1
        ...
        if back_test == 0: #（向前模拟） 这些值用作模拟的起点，以预测未来的变量变化。
            #
            ...
            #（历史回测）。这些值用作模拟的起点，以检查模型能否从历史数据中准确重建经济变量
...
#SIM_X 是一个三维数组，维度为 [NumOfSim, NumOfTime+1, self.NumV]。
#NumOfSim: 模拟路径数量。  NumOfTime+1: 时间步长（包括初始值）。 self.NumV: 模拟的变量数量，共 14 个变量
...
#长期利率 r_longTerm 和短期利率 r_shortTerm: 初始值受历史通胀影响。
...
# 利用 np.random.randn(NumOfSim) 为每个模拟路径生成随机变量，用于引入不确定性。
...
#有公式提前写好的
           ...
           #保存上一时间步的变量值（如 pre_lnoq, pre_snoq），供下一步计算使用。
        ...
        self.SIM_X = SIM_X[:,1:,:]
        self.SIM_X_full = SIM_X[:,:,:] #SIM_X 中 完整结果（包含初始值）。
